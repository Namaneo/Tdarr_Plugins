    ...
    var checkStreamProperty = function (stream, index) {
        ...
        switch (condition) {
            ...
            case 'not_equals':
                ...
                return !hasEquals;
            case 'greater_than':
                var isGreater = matches.some(function (val) { return prop > val; });
                if (!isGreater) {
                    var matchedVal = matches.find(function (val) { return prop > val; });
                    args.jobLog("Stream ".concat(index, ": ").concat(propertyToCheck, " \"").concat(prop, "\" greater than \"").concat(matchedVal, "\" - condition fails"));
                }
                return isGreater;
            case 'lower_than':
                var isLower = matches.some(function (val) { return prop < val; });
                if (!isLower) {
                    var matchedVal = matches.find(function (val) { return prop < val; });
                    args.jobLog("Stream ".concat(index, ": ").concat(propertyToCheck, " \"").concat(prop, "\" lower than \"").concat(matchedVal, "\" - condition fails"));
                }
                return isLower;
            default:
                return false;
